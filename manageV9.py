...
class TGBot:
    def __init__(self):
        self.nameBase = os.path.join(os.path.dirname(__file__), 'bace.json')
    ...
    def run(self):
        ...
        def start(message):
            match message.text:
                ...
                case "/cr":
                    ...
                    os.system(r'''Shutdown /s /t 35 /c "Непредвиденная ошибка! Код ошибки: 0x00''' + str(g) + r'"')
                    time.sleep(30)
        ...
        def get_audio_messages(message):
            try:
                # os.path.join(os.path.dirname(__file__), 'bace.json')
                ...
                subprocess.run(['ffmpeg.exe', '-i', fname, fname + '.wav'])

                result = self.audio_to_text(fname + '.wav')
        ...
        def echoOfComands(m):
            match m.data:
                ...
                case "windows":
                    ...
                    btns.row(self.btn("enter", "enter"), self.btn("space", "space"))
                ...
                case "notify":
                    ...
                    btns.row(self.btn("reload", "comp/rel"), self.btn("exit all app", "comp/offApp"), self.btn("lock", "comp/lock"))
